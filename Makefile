...
sync-with-handheld:
	rsync --archive --delete --progress --human-readable "${ROOT_DIR}" "${DEVICE_SSH_URI}":"${DEVICE_THEME_PATH}" && ssh "${DEVICE_SSH_URI}" '${COMMAND_REMOTE_EMULATIONSTATION_RELOAD}'

# Requires `watchexec` and `sshpass`
sync-with-handheld-unattended:
	watchexec -- sshpass -p "${DEVICE_SSH_PASS}" rsync --archive --delete --progress --human-readable "${ROOT_DIR}" "${DEVICE_SSH_URI}":"${DEVICE_THEME_PATH}" \&\& sshpass -p "${DEVICE_SSH_PASS}" ssh "${DEVICE_SSH_URI}" '${COMMAND_REMOTE_EMULATIONSTATION_RELOAD}'
